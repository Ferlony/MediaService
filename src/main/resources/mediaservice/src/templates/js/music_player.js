...
function simp_initTime() {
  ...
  if (simp_audio.currentTime == simp_audio.duration) {
    simp_controls.querySelector('.simp-plause').classList.remove('button-pause');
    simp_controls.querySelector('.simp-plause').classList.add('button-play');
...
//   simp_source[simp_a_index].parentNode.scrollTop = simp_active.top;
  ...
  if (simp_isPlaying) {
    simp_controls.querySelector('.simp-plause').classList.remove('button-play');
    simp_controls.querySelector('.simp-plause').classList.add('button-pause');
  }
}
...
function simp_startScript() {
  ...
  simp_controls.querySelector('.simp-plauseward').addEventListener('click', function(e) {
    ...
    if (eles.contains('simp-plause')) {
      if (simp_audio.paused) {
        ...
        simp_audio.play();
        simp_isPlaying = true;
        eles.remove('button-play');
        eles.add('button-pause');
      } else {
        simp_audio.pause();
        simp_isPlaying = false;
        eles.remove('button-pause');
        eles.add('button-play');
      }
    } else {
  ...
  simp_volume.addEventListener('click', function(e) {
    ...
    if (eles.contains('simp-mute')) {
      if (eles.contains('button-volume')) {
        eles.remove('button-volume');
        eles.add('button-mute');
        simp_v_slider.value = 0;
      } else {
        eles.remove('button-mute');
        eles.add('button-volume');
        simp_v_slider.value = simp_v_num;
      }
    } else {
      simp_v_num = simp_v_slider.value;
      if (simp_v_num != 0) {
        simp_controls.querySelector('.simp-mute').classList.remove('button-mute');
        simp_controls.querySelector('.simp-mute').classList.add('button-volume');
      }
    }
...
if (document.querySelector('#simp')) {
  ...
  simp_elem += '<div class="simp-plauseward flex flex-align"><button type="button" class="simp-prev fa button-back" disabled></button><button type="button" class="simp-plause fa button-play" disabled></button><button type="button" class="simp-next fa button-next" disabled></button></div>';
  ...
  simp_elem += '<div class="simp-volume flex flex-align"><button type="button" class="simp-mute fa button-volume"></button><input class="simp-v-slider" type="range" min="0" max="100" value="100"/></div>';
  simp_elem += '<div class="simp-others flex flex-align"><button type="button" class="simp-random fa button-shuffle" title="Random"></button>';
  simp_elem += '</div>'; //simp-controls
// <button type="button" class="simp-plext fa button-play-circle" title="Auto Play"></button>  <div class="simp-shide"><button type="button" class="simp-shide-top fa fa-caret-up" title="Show/Hide Album"></button><button type="button" class="simp-shide-bottom fa fa-caret-down" title="Show/Hide Playlist"></button></div></div>
